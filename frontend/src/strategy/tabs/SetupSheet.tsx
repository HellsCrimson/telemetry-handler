// SetupSheet shows the player car's active setup — aero, suspension, gearing,
// tyres, brakes, engine maps — read live from LMU's REST API (GetCarSetup). This
// is the one strategy view NOT fed by the per-frame SessionState: the setup only
// changes in the garage, so it is fetched on demand (on mount + a manual refresh)
// rather than polled. Until now the setup was unavailable to the app at all (the
// shared-memory telemetry doesn't carry it), which is why the balance read is a
// heuristic and Car Management couldn't show ARB / setup values.
        ...
        <div>
          {setup ? (
            <>
              ...
              {setup.fixed_setup_race && <span className="strat-setup-badge fixed">fixed setup</span>}
              <p className="muted">
